...
import { ConfidenceBadge, EntityId, StatusBadge, TraceabilityChain } from '../components/TraceabilityUI';
...
function TraceabilityView({ data }: { data: WorkflowResult }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-border/80 bg-card shadow-sm"><div className="border-b border-border p-5"><h2 className="font-semibold">Requirement → User Story → Test Scenario ID → Test Case ID</h2><p className="mt-1 text-sm text-muted-foreground">Every row preserves the identifiers returned by the generation workflow.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="p-4">Coverage</th><th className="p-4">Requirement</th><th className="p-4">User story</th><th className="p-4">Traceability chain</th><th className="p-4">Generated asset</th></tr></thead><tbody>{rows.map(({ testCase, scenario }, index) => { const covered = Boolean(testCase.requirement_ids?.length && scenario); return <tr key={`${testCase.test_case_id}-${index}`} className="border-t border-border transition-colors hover:bg-muted/30"><td className="p-4"><StatusBadge status={covered ? 'fully covered' : 'partial / orphan'} /></td><td className="p-4">{testCase.requirement_ids?.join(', ') || 'Unmapped'}</td><td className="p-4">{scenario?.user_story_ids?.join(', ') || 'Unmapped'}</td><td className="p-4"><TraceabilityChain scenarioId={testCase.scenario_id} testCaseId={testCase.test_case_id} /></td><td className="p-4"><p className="font-medium">{testCase.title}</p><p className="mt-1 text-xs text-muted-foreground">{scenario?.title || 'Scenario record unavailable'}</p></td></tr>; })}</tbody></table></div></section>;
}
