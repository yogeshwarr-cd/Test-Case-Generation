...
class ScenarioValidationAgent(BaseAgent[ValidationResult]):
    ...
    async def run(self,input_data,execution_context:ExecutionContext)->ValidationResult:
        ...
        duplicates=duplicate_indexes([s.title for s in ss]); types={s.scenario_type for s in ss}; complete=[s for s in ss if s.description and s.expected_business_outcome and s.preconditions]
