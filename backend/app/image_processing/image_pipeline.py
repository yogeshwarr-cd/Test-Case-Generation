...
class ImagePipeline:
    _locks={}
    async def analyze(self,content,mime_type,original_name,image_description="",project_id=None,confidence_threshold=None):
        if Path(original_name).suffix.lower() not in {".png",".jpg",".jpeg",".webp"}: raise ValueError("Unsupported image file extension")
        threshold=float(settings.vision_llm_min_local_confidence if confidence_threshold is None else confidence_threshold)
        image,checksum=validate_and_decode(content,mime_type);root=Path(settings.image_storage_path).resolve();root.mkdir(parents=True,exist_ok=True);cache_key=f"{checksum}-{round(threshold*10000):04d}";cache=root/f"{cache_key}.json"
        ...
        if overall<threshold: await VisionFallback().analyze(checksum,normalized)
        if blank:warnings.append("Image appears blank or unreadable")
        if overall<threshold:warnings.append(f"Visual analysis confidence is below the workflow threshold of {threshold*100:g}%; manual review recommended")
        ...
        now=datetime.now(timezone.utc).isoformat();record={"image_id":image_id,"checksum":checksum,"confidence_threshold":threshold,"original_name":Path(original_name).name,"safe_file_name":safe_path.name,"mime_type":mime_type,"file_size":len(content),"storage_path":str(safe_path),"project_id":str(project_id) if project_id else None,"upload_status":"stored","analysis_status":"analyzed","created_at":now,"updated_at":now,"full_analysis":full,"compact_context":{**compact,"overall_confidence":overall,"confidence_threshold":threshold,"warnings":warnings}}
        cache.write_text(json.dumps(record),encoding="utf-8");(root/f"{image_id}.ref").write_text(cache_key,encoding="utf-8");return record,False
    @staticmethod
    def get(image_id):
