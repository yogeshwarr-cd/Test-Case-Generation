...
Keep the required provider and optional Seacrawl keys configured, then restart the backend. Confirm `GET /health` returns `"mode": "live"` and `GET /api/v1/automation/health` reports the real browser/service state.
