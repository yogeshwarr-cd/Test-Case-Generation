...
class TestCaseValidationAgent(BaseAgent[ValidationResult]):
    ...
    async def run(self,input_data,execution_context:ExecutionContext)->ValidationResult:
        ...
        issues=[ValidationIssue(issue_code="DUPLICATE_TESTCASE",description="Test case duplicates another",affected_entity_id=cases[i].test_case_id,recommendation="Regenerate this test case only") for i in duplicates]; score=weighted_score(vals,TESTCASE_WEIGHTS)
