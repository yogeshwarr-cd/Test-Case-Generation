...
async def regenerate_scenarios_node(s): return await generate_scenarios_node(s)
...
async def generate_test_cases_node(s): s["status"]=s["current_stage"]="generating_test_cases"; s["test_cases"]=(await TestCaseGenerationAgent().execute({"scenarios":s["scenarios"]},_ctx(s))).model_dump(mode="json")["test_cases"]; return s
...
async def regenerate_test_cases_node(s): s["test_cases"]=[c for c in s["test_cases"] if c["test_case_id"] not in set(s["testcase_validation"].get("failed_entity_ids",[]))]; return await generate_test_cases_node(s)
