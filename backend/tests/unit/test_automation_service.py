...
async def test_generation_reads_completed_workflow_without_mutating_it(monkeypatch, tmp_path):
    ...
    # Validated requirement traceability is preserved through UI-derived
    # generation; the script must retain the originating scenario ID.
    assert response.scripts[0].scenario_id == "SC-1"
    # The generated artifact intentionally retains test-case traceability.
    assert "TC-1" in response.scripts[0].source
